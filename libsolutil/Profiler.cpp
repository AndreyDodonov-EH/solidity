...
util::Profiler::Probe::~Probe()
{
	...
	auto [metricsIt, inserted] = Profiler::singleton().s_metrics.try_emplace(m_scopeName, Metrics{0, 0});
...
{
	std::vector<std::pair<std::string, Metrics>> sortedMetrics(s_metrics.begin(), s_metrics.end());
	std::sort(
		sortedMetrics.begin(),
		sortedMetrics.end(),
