...
util::Profiler::Probe::~Probe()
{
	...
	auto [durationIt, inserted] = Profiler::singleton().s_durations.try_emplace(m_scopeName, 0);
...
{
	std::vector<std::pair<std::string, int64_t>> durations(s_durations.begin(), s_durations.end());
	std::sort(
		durations.begin(),
		durations.end(),
